...
export function init(appSettings, currentUser) { // currentUser is now used for permissions
    ...
    async function loadSettings() {
        try {
            ...
            // Load permissions for the currently logged-in user (Admin role)
            // This section is for displaying/editing the permissions of the user
            // who is currently logged in and viewing the settings page.
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (currentUser && currentUser.role === 'Admin' && permissionsContainer) {
                ...
                // Rebuild sidebar navigation to reflect permission changes immediately
                // This requires buildSidebarNav to be accessible, typically from renderer.js
                // For simplicity, we can trigger a full reload to apply permission changes.
                // Or, if buildSidebarNav is exposed, call it directly.
                // window.location.reload(); // Consider a full reload if sidebar doesn't update automatically
            }
