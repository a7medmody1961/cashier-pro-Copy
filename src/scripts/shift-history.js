...
    const shiftExpensesTableBody = document.getElementById('shift-expenses-table-body');
    ...
    // ADD THIS TEST CALL
    try {
        const testResult = await window.api.testShiftConnection();
        console.log("Test Connection Result (Renderer):", testResult); // سيظهر هذا في كونسول أدوات المطور (Developer Tools)
    } catch (error) {
        console.error("Test Connection Error (Renderer):", error); // سيظهر هذا في كونسول أدوات المطور
    }
...
                        row.innerHTML = `
                            <td>${sale.id}</td>
                            <td>${sale.customer_id ? `ID: ${sale.customer_id}` : 'لا يوجد'}</td>                             <td>${sale.total_amount.toFixed(2)}</td>
